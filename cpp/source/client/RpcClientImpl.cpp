...
void RpcClientImpl::asyncSyncLiteSubscription(const SyncLiteSubscriptionRequest& request,
                                              InvocationContext<SyncLiteSubscriptionResponse>* invocation_context) {
  std::weak_ptr<RpcClient> rpc_client(shared_from_this());
  auto callback = std::bind(&RpcClientImpl::asyncCallback, rpc_client, invocation_context, std::placeholders::_1);
  stub_->async()->SyncLiteSubscription(&invocation_context->context, &request, &invocation_context->response, callback);
}
