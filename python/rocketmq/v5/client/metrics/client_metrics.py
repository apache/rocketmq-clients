...
from rocketmq.v5.model import HistogramEnum, MetricContext, MessageMetricType
...
class ClientMetrics:
    ...
    def send_after(self, send_context: MetricContext, success: bool):
        ...
        if send_context.metric_type != MessageMetricType.SEND:
            logger.warn(f"metric type must be MessageMetricType.SEND. current send_context type is {send_context.metric_type}")
            return
    ...
    def __meter_provider_start(self):
        ...
        try:
            # setup OTLP exporter
            exporter = OTLPMetricExporter(endpoint=self.__endpoints.__str__(), insecure=True, timeout=ClientMetrics.METRIC_EXPORTER_RPC_TIMEOUT)

            # create a metric reader and set the export interval
            reader = PeriodicExportingMetricReader(exporter, export_interval_millis=ClientMetrics.METRIC_READER_INTERVAL)

            # create an empty resource
            resource = Resource.get_empty()

            # create view
            send_cost_time_view = View(instrument_type=Histogram, instrument_name=HistogramEnum.SEND_COST_TIME.histogram_name, aggregation=ExplicitBucketHistogramAggregation(HistogramEnum.SEND_COST_TIME.buckets))

            # create MeterProvider
            self.__meter_provider = MeterProvider(metric_readers=[reader], resource=resource, views=[send_cost_time_view])

            # define the histogram instruments
            self.__send_success_cost_time_instrument = self.__meter_provider.get_meter(ClientMetrics.METRIC_INSTRUMENTATION_NAME).create_histogram(HistogramEnum.SEND_COST_TIME.histogram_name)
