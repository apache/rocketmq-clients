...
from rocketmq.v5.exception import IllegalStateException, IllegalArgumentException
from rocketmq.v5.util import ClientId, Misc, ConcurrentMap, MessagingResultChecker, Signature
from rocketmq.v5.client.connection import RpcClient
from rocketmq.v5.model import TopicRouteData, CallbackResult, CallbackResultType
from rocketmq.grpc_protocol import Code, ClientType
from rocketmq.grpc_protocol import QueryRouteRequest
...
class Client:
    ...
    def __init__(self, client_configuration, topics, client_type: ClientType, tls_enable=False):
        ...
        if topics is not None:
            self.__topics = set(filter(lambda topic: Misc.is_valid_topic(topic), topics))
        else:
            self.__topics = set()
    ...
    def startup(self):
        try:
            if self.__had_shutdown is True:
                raise Exception(f"client:{self.__client_id} had shutdown, can't startup again.")
            ...
            try:
                ...
                logger.warn(f"update topic exception when client startup, ignore it, try it again in scheduler. exception: {e}")
    ...
    def __start_scheduler(self):
        ...
        try:
            ...
            self.__topic_route_scheduler = threading.Thread(target=self.__schedule_update_topic_route_cache, name="update_topic_route_schedule_thread")
            ...
            self.__heartbeat_scheduler = threading.Thread(target=self.__schedule_heartbeat, name="heartbeat_schedule_thread")
            ...
            self.__sync_setting_scheduler = threading.Thread(target=self.__schedule_update_setting, name="sync_setting_schedule_thread")
            ...
            self.__clear_idle_rpc_channels_scheduler = threading.Thread(target=self.__schedule_clear_idle_rpc_channels, name="clear_idle_rpc_channel_schedule_thread")
            ...
            if self.__client_thread_task_enabled is True:
                ...
                for topic in topics:
                    try:
                        ...
                        logger.error(f"{self.__str__()} run scheduler for update topic:{topic} route cache exception: {e}")
            else:
                break
    ...
    def __schedule_clear_idle_rpc_channels(self):
        while True:
            if self.__client_thread_task_enabled is True:
                ...
                logger.debug(f"{self.__str__()} run scheduler for clear idle rpc channels.")
                try:
                    if self.__client_thread_task_enabled is True:
                        self.__rpc_client.clear_idle_rpc_channels()
                except Exception as e:
                    logger.error(f"{self.__str__()} run scheduler for clear idle rpc channels: {e}")
            else:
                break
        logger.info(f"{self.__str__()} stop scheduler for clear idle rpc channels success.")
    ...
    def __handle_callback(self):
        while True:
            if self.__client_thread_task_enabled is True:
                ...
                if callback_result.result_type == CallbackResultType.END_CALLBACK_THREAD_RESULT:
    ...
    def __heartbeat_async(self, endpoints):
        ...
        future = self.__rpc_client.heartbeat_async(endpoints, req, metadata=self._sign(),
                                                   timeout=self.__client_configuration.request_timeout)
        future.add_done_callback(callback)
    ...
    def __heartbeat_callback(self, future, endpoints):
        try:
            ...
            if res is not None and res.status.code == Code.OK:
                logger.info(f"{self.__str__()} send heartbeat to {endpoints.__str__()} success.")
            else:
                if res is not None:
                    logger.error(f"{self.__str__()} send heartbeat to {endpoints.__str__()} error, code:{res.status.code}, message:{res.status.message}.")
                else:
                    logger.error(f"{self.__str__()} send heartbeat to {endpoints.__str__()} error, response is none.")
        except Exception as e:
            logger.error(f"{self.__str__()} send heartbeat to {endpoints.__str__()} exception, e: {e}")
            raise e
    ...
    def __retrieve_telemetry_stream_stream_call(self, endpoints, rebuild=False):
        try:
            self.__rpc_client.telemetry_stream(endpoints, self, metadata=self._sign(), timeout=60 * 60 * 24 * 365, rebuild=rebuild)
        except Exception as e:
            logger.error(f"{self.__str__()} rebuild stream_steam_call to {endpoints.__str__()} exception: {e}" if rebuild else f"{self.__str__()} create stream_steam_call to {endpoints.__str__()} exception: {e}")
    ...
    def __setting_write_callback(self, future, endpoints):
        try:
            ...
            logger.warn(f"{self.__str__()} send setting to {endpoints.__str__()} occurred AioRpcError: {e}")
            self.__retrieve_telemetry_stream_stream_call(endpoints, rebuild=True)
        except Exception as e:
            logger.error(f"{self.__str__()} send setting to {endpoints.__str__()} exception: {e}")
            self.__retrieve_telemetry_stream_stream_call(endpoints, rebuild=True)
    ...
    def __client_termination(self, endpoints):
        ...
        future = self.__rpc_client.notify_client_termination(endpoints, req, metadata=self._sign(),
                                                             timeout=self.__client_configuration.request_timeout)
        future.result()
    ...
    def __check_topic_route_endpoints_changed(self, topic, route):
        ...
        if old_route is None or old_route != route:
            logger.info(f"topic:{topic} route changed for {self.__str__()}. old route is {old_route}, new route is {route}")
        all_endpoints = self.__get_all_endpoints()  # the existing endpoints
        topic_route_endpoints = route.all_endpoints()  # the latest endpoints for topic route
        diff = set(topic_route_endpoints.keys()).difference(set(all_endpoints.keys()))  # the diff between existing and latest
