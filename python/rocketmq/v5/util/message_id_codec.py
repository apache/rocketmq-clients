    ...
    def __init__(self):
        with MessageIdCodec._instance_lock:
            if not hasattr(self, "_initialized"):
                ...
                self.__process_fixed_string_v1 = buffer.hex().upper()
                self.__seconds_since_custom_epoch = int(
                    (
                        datetime.now(timezone.utc)
                        - datetime(2021, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
                    ).total_seconds()
                )
                self.__seconds_start_timestamp = int(time())
                self.__seconds = self.__delta_time()
                self._initialized = True

    def next_message_id(self):
        delta_seconds = self.__delta_time()
        if self.__seconds != delta_seconds:
            self.__seconds = delta_seconds
        buffer = bytearray(8)
        buffer[0:4] = self.__seconds.to_bytes(8, byteorder="big")[4:8]
        buffer[4:8] = MessageIdCodec.__index.get_and_increment().to_bytes(4, byteorder="big")
        return (
            MessageIdCodec.MESSAGE_ID_VERSION_V1
            + self.__process_fixed_string_v1
            + buffer.hex().upper()
        )

    def __delta_time(self):
        return (
            int(time()) - self.__seconds_start_timestamp + self.__seconds_since_custom_epoch
        )
