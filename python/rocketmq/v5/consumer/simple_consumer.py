...
from rocketmq.v5.client import *
from rocketmq.grpc_protocol import ReceiveMessageRequest, TelemetryCommand, HeartbeatRequest,\
    AckMessageRequest, AckMessageEntry, ChangeInvisibleDurationRequest, NotifyClientTerminationRequest, ClientType, Settings, Subscription
from rocketmq.v5.util import Misc, AtomicInteger, ConcurrentMap, MessagingResultChecker
from rocketmq.v5.model import Message, FilterExpression, CallbackResult
...
class SimpleConsumer(Client):
    ...
    def __init__(self, client_configuration: ClientConfiguration, consumer_group, subscription: dict = None, await_duration=20):
        ...
        if Misc.is_valid_consumer_group(consumer_group) is False:
            raise IllegalArgumentException(f"consumerGroup does not match the regex [regex={Misc.CONSUMER_GROUP_PATTERN}]")
        if await_duration is None:
            raise IllegalArgumentException("awaitDuration should not be null")

        super().__init__(client_configuration, None if subscription is None else subscription.keys(), ClientType.SIMPLE_CONSUMER)
    ...
    def __select_topic_queue(self, topic):
        try:
            ...
            queue_selector = self.__receive_queue_selectors.put_if_absent(topic, QueueSelector.simple_consumer_queue_selector(route))
    ...
    def __receive(self, max_message_num, invisible_duration):
        ...
        read_future = asyncio.run_coroutine_threadsafe(self.__receive_message_response(future.result()), self._rpc_channel_io_loop())
    ...
    def __handle_receive_message_response(self, responses):
        ...
        for res in responses:
            if res.HasField("status"):
                logger.debug(f"simple_consumer[{self.__consumer_group}] receive_message, code:{res.status.code}, message:{res.status.message}.")
                status = res.status
    ...
    def __handle_change_invisible_result(self, future, ret_future=None):
        try:
            ...
            MessagingResultChecker.check(res.status)
            if ret_future is not None:
                self._set_future_callback_result(CallbackResult.async_change_invisible_duration_callback_result(ret_future, None))
        except Exception as e:
            if ret_future is None:
                raise e
            else:
                self._set_future_callback_result(CallbackResult.async_change_invisible_duration_callback_result(ret_future, e, False))
