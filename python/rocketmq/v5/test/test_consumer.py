...
from rocketmq.v5.model import Message, FilterExpression
...
class TestNormalConsumer(unittest.TestCase):
    ...
    @patch.object(SimpleConsumer, '_SimpleConsumer__select_topic_queue', return_value=TestBase.fake_queue(TestBase.FAKE_TOPIC_0))
    @patch.object(SimpleConsumer, '_SimpleConsumer__select_topic_for_receive', return_value=TestBase.FAKE_TOPIC_0)
    ...
    def test_receive(self, mock_update_topic_route, mock_start_scheduler, mock_select_topic_for_receive, mock_select_topic_queue, mock_receive_message_async, mock_receive_message_response, mock_message_body_check_sum):
        future = Future()
        ...
        consumer = SimpleConsumer(TestBase.fake_client_config(), TestBase.FAKE_CONSUMER_GROUP_0, subs)
        consumer.startup()
