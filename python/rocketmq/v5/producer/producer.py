...
from rocketmq.v5.client.client import *
from rocketmq.grpc_protocol import ClientType, Settings, Publishing, MessageType, \
    TransactionResolution, TransactionSource, Code, SendMessageRequest, TelemetryCommand, HeartbeatRequest, \
    EndTransactionRequest, NotifyClientTerminationRequest, Encoding
from rocketmq.v5.util import MessageIdCodec, ConcurrentMap, MessagingResultChecker, Misc
from rocketmq.v5.client.balancer import QueueSelector
from rocketmq.v5.exception import *
from rocketmq.v5.model import *
    ...
    def __commit_or_rollback(self, result):
        ...
        try:
            res = self.__producer.end_transaction(self.__send_receipt.message_queue.endpoints, self.__message,
                                                  self.__send_receipt.transaction_id, result, TransactionSource.SOURCE_CLIENT)
            if res.status.code != Code.OK:
                logger.error(f"transaction commit or rollback error. topic:{self.__message.topic}, message_id:{self.__message.message_id}, transaction_id:{self.__send_receipt.transaction_id}, transactionResolution:{result}")
                raise ClientException(res.status.message, res.status.code)
            return res
        except Exception as e:
            logger.error(f"end transaction error, topic:{self.__message.topic}, message_id:{self.__send_receipt.message_id}, transaction_id:{self.__send_receipt.transaction_id}, transactionResolution:{result}: {e}")
            raise e
...
class Producer(Client):
    ...
    def send(self, message: Message, transaction=None) -> SendReceipt:
        ...
        if message.message_type not in topic_queue.accept_message_types:
            raise IllegalArgumentException(f"current message type not match with queue accept message types, topic:{message.topic}, message_type:{message.message_type}, queue access type:{topic_queue.accept_message_types}")
    ...
    async def on_recover_orphaned_transaction_command(self, endpoints, msg, transaction_id):
        ...
        try:
            if self.is_running is False:
                raise IllegalStateException("unable to recover orphaned transaction command because producer is not running")
            ...
            if result == TransactionResolution.COMMIT:
                res = await self.__commit_for_server_check(endpoints, message, transaction_id, TransactionSource.SOURCE_SERVER_CHECK)
                logger.debug(f"commit message. message_id: {message.message_id}, transaction_id: {transaction_id}, res: {res}")
            elif result == TransactionResolution.ROLLBACK:
                res = await self.__rollback_for_server_check(endpoints, message, transaction_id, TransactionSource.SOURCE_SERVER_CHECK)
                logger.debug(f"rollback message. message_id: {message.message_id}, transaction_id: {transaction_id}, res: {res}")
        except Exception as e:
    ...
    def __send_async(self, message: Message, topic_queue, attempt=1, ret_future=None):
        ...
        if ret_future is None:
            ret_future = Future()
        handle_send_receipt_callback = functools.partial(self.__handle_async_send_receipt, message=message, topic_queue=topic_queue, attempt=attempt, ret_future=ret_future, send_metric_context=send_context)
        send_message_future.add_done_callback(handle_send_receipt_callback)
        return ret_future


    def __handle_async_send_receipt(self, send_message_future, message, topic_queue, attempt, ret_future, send_metric_context=None):
        try:
            send_receipt = self.__process_send_message_response(send_message_future, topic_queue)
            self.client_metrics.send_after(send_metric_context, True)
            ...
            if retry_exception_future is not None:
                # end retry with exception
                self.client_metrics.send_after(send_metric_context, False)
                self._set_future_callback_result(CallbackResult.async_send_callback_result(ret_future, retry_exception_future.exception(), False))
                return
            # resend message
            topic_queue = self.__select_send_queue(message.topic)
    ...
    def __process_send_message_response(self, send_message_future, topic_queue):
        ...
        entries = res.entries
        assert len(entries) == 1, f"entries size error, the send response entries size is {len(entries)}, {self.__str__()}"
        entry = entries[0]
    ...
    def __check_send_retry_condition(self, message, topic_queue, attempt, e):
        ...
        if attempt > Producer.MAX_SEND_ATTEMPTS:
            logger.error(f"{self.__str__()} failed to send message to {topic_queue.endpoints.__str__()}, because of run out of attempt times, topic:{message.topic}, message_id:{message.message_id},  message_type:{message.message_type}, attempt:{attempt}")
            end_retry = True

        # no need more attempts for transactional message
        if message.message_type == MessageType.TRANSACTION:
            logger.error(f"{self.__str__()} failed to send message to {topic_queue.endpoints.__str__()}, topic:{message.topic}, message_id:{message.message_id}, message_type:{message.message_type} ,attempt:{attempt}")
            end_retry = True

        # end retry if system busy
        if isinstance(e, TooManyRequestsException):
            logger.error(f"{self.__str__()} failed to send message to {topic_queue.endpoints.__str__()}, because of to too many requests, topic:{message.topic},  message_type:{message.message_type}, message_id:{message.message_id}, attempt:{attempt}")
            end_retry = True
    ...
    def __send_req(self, message: Message):
        try:
            ...
            if len(message.body) > max_body_size:
                raise IllegalArgumentException(f"Message body size exceeds the threshold, max size={max_body_size} bytes")
    ...
    def __select_send_queue(self, topic):
        try:
            ...
            queue_selector = self.__send_queue_selectors.put_if_absent(topic, QueueSelector.producer_queue_selector(route))
    ...
    def __commit_for_server_check(self, endpoints, message: Message, transaction_id, source):
        return self.__end_transaction_for_server_check(endpoints, message, transaction_id, TransactionResolution.COMMIT, source)


    def __rollback_for_server_check(self, endpoints, message: Message, transaction_id, source):
        return self.__end_transaction_for_server_check(endpoints, message, transaction_id, TransactionResolution.ROLLBACK, source)


    def __end_transaction_for_server_check(self, endpoints, message: Message, transaction_id, result, source):
        req = self.__end_transaction_req(message, transaction_id, result, source)
        return self.rpc_client.end_transaction_for_server_check(endpoints, req, metadata=self._sign(), timeout=self.client_configuration.request_timeout)
