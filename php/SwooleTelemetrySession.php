...
class SwooleTelemetrySession {
    ...
    private function createStreamAndSync(): void {
        ...
        // Upgrade to HTTP/2 for gRPC bidirectional stream
        $path = '/apache.rocketmq.v2.MessagingService/Telemetry';
        $upgradeResult = $httpClient->upgrade($path);
        
        if (!$upgradeResult) {
            throw new \Exception(
                "Failed to upgrade to HTTP/2: errCode=" . $httpClient->errCode . 
        ...
        Logger::info("HTTP/2 stream established, path={$path}, clientId={$this->clientId}");
