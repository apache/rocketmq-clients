...
public class ProcessQueueTest extends BaseConfig {
  ...
  public void testPopMessage() throws MQClientException {
    ...
    Mockito.when(mockedConsumerImpl.getClientInstance()).thenReturn(mockedClientInstance);
    Mockito.when(mockedClientInstance.resolveTarget(anyString(), anyString()))
...

    {
      ...
      Mockito.when(
              mockedClientInstance.popMessageAsync(
                  anyString(), any(PopMessageRequest.class), anyLong(), any(TimeUnit.class)))
          .thenReturn(future);
