...
public class ClientInstance {
    public static final long RPC_DEFAULT_TIMEOUT_MILLIS = 3 * 1000;
    ...
    public long queryOffset(RpcTarget target, QueryOffsetRequest request) throws MQClientException, MQServerException {
        final RpcClient rpcClient = this.getRpcClient(target);
        final QueryOffsetResponse response = rpcClient.queryOffset(request, RPC_DEFAULT_TIMEOUT_MILLIS,
                                                                   TimeUnit.MILLISECONDS);
        final Status status = response.getCommon().getStatus();
        final int code = status.getCode();
        if (Code.OK_VALUE != code) {
            throw new MQServerException("Failed to query offset from remote");
        }
        return response.getOffset();
    }

    public PullResult pullMessage(RpcTarget target, PullMessageRequest request,
                                  long duration, TimeUnit unit) throws MQClientException {
        final RpcClient rpcClient = this.getRpcClient(target);
        final PullMessageResponse response = rpcClient.pullMessage(request, duration, unit);
        return processPullMessageResponse(target, response);
    }

    public ListenableFuture<PullResult> pullMessageAsync(final RpcTarget target, PullMessageRequest request,
                                                         long duration, TimeUnit unit) throws MQClientException {
        final RpcClient rpcClient = this.getRpcClient(target);
        final ListenableFuture<PullMessageResponse> future =
                rpcClient.pullMessage(request, asyncRpcExecutor, duration, unit);
        return Futures.transform(
                future,
                new Function<PullMessageResponse, PullResult>() {
                    @Override
                    public PullResult apply(PullMessageResponse response) {
                        return processPullMessageResponse(target, response);
                    }
                });
    }
    ...
    public static PopResult processReceiveMessageResponse(RpcTarget target, ReceiveMessageResponse response) {
        ...
        switch (code != null ? code : Code.UNKNOWN) {
            ...
            default:
                ...
                log.warn(
                        "Pop response indicated server-side error, server endpoints={}, code={}, status message={}",
                        target.getEndpoints(), code, status.getMessage());
        }
        ...
        if (PopStatus.OK == popStatus) {
            ...
            for (Message message : messageList) {
                try {
                    MessageImpl messageImpl = wrapMessageImpl(message);
                    messageImpl.getSystemAttribute().setAckRpcTarget(target);
                    msgFoundList.add(new MessageExt(messageImpl));
                } catch (MQClientException e) {
