...
describe('Endpoints.getGrpcTarget uses the custom ip scheme', () => {
  it('emits ip: scheme for IPv4/IPv6 endpoints', () => {
    ...
    // Bracketed IPv6 string parsing lands with the endpoints-parsing fix; build
    // the IPv6 endpoints from protobuf so this test only covers the scheme.
    const pb = new EndpointsPB();
    pb.setScheme(AddressScheme.IPV6);
    pb.addAddresses().setHost('::1').setPort(10911);
    assert.ok(new Endpoints(pb.toObject()).getGrpcTarget().startsWith('ip:'));
  });
